    ...
    : GenericRepository<ShopReview>(context), IShopReviewRepository
{
    ...
    {
        var shopReview = await context.ShopReviews.Include(p => p.User).FirstOrDefaultAsync(r => r.Id == id  && !r.Shop.IsDeleted);
        if (shopReview == null)
        {
    ...
    {
        var shopReview = await context.ShopReviews.FirstOrDefaultAsync(r => r.Id == shopReviewId && !r.Shop.IsDeleted);
        if (shopReview == null)
        {
            throw new NotFoundException("Shop review not found");
        }
    ...
    {
        var shopReviews = await context.ShopReviews.Where(r => r.ShopId == shopId  && !r.Shop.IsDeleted && !r.IsDeleted).ToListAsync();
    ...
    {
        return await context.ShopReviews.CountAsync(r => r.ShopId == shopId  && !r.Shop.IsDeleted && !r.IsDeleted);
    }
}