...
public class UpdateShopHandler(IUnitOfWork unitOfWork, IMapper mapper, IImageRepository imageRepository, IAzureBlobStorageService azureBlobStorageService)
    : IRequestHandler<UpdateShopRequest, BaseResponse<ShopResponseDTO>>
{
    public async Task<BaseResponse<ShopResponseDTO>> Handle(UpdateShopRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            string base64Image = request.Shop.Banner.Split(',')[1];
        ...
        {
            string base64Image = request.Shop.Logo.Split(',')[1];
            byte[] imageBytes = Convert.FromBase64String(base64Image);
            shop.Logo = await azureBlobStorageService.UpdateImageAsync(imageBytes, shop.Id + "-logo"); 
