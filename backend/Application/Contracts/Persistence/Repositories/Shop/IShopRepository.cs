...
public interface IShopRepository: IGenericRepository<Domain.Entities.Shop.Shop>
{
    Task<IReadOnlyList<Domain.Entities.Shop.Shop>> GetShopListAsync(
        string? search,  List<string>? category,    List<WorkingHourQueryParamater>? workingHours,   double? radiusInKilometers, double? latitude, double? longitude,
    ...
    Task<Domain.Entities.Shop.Shop> GetAllShopByIdAsync(string id);
    ...
    Task<bool> IsShopDeletedAsync(string shopId);
    ...
    Task<IReadOnlyList<string>> GetShopVideosAsync(string shopId,int skip = 0,
        int limit = 10);
    Task<bool> FollowShopAsync(string shopId, string userId);
    ...
    Task<int> CountReviewStart(string shopId, int start);
    Task<List<Domain.Entities.Shop.Shop>> GetVerificationReviewPendingShopsAsync(int skip = 0, int limit = 10);
