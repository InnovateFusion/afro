...
namespace backend.WebApi.Controllers.Common
{
    ...
    public class ImageController(IMediator mediator) : ControllerBase
    {
        ...
        [Authorize]  
        public async Task<IActionResult> RemoveBackground([FromForm] ImageFileUploadDto imageModel)
        {
                var imageStream = await mediator.Send(new BackgroundRemoverRequest { Image = imageModel });
                return File(imageStream, "image/png", "output.png");
        }
        
        [HttpPost("Remove-Background-From-Url")]
        [Authorize]
        public async Task<IActionResult> RemoveBackgroundFromUrl([FromBody] ImageUrlDto imageUrl)
        {
            var imageStream = await mediator.Send(new BackgroundRemoverFromUrlRequest { ImageUrl = imageUrl.ImageUrl });
            return File(imageStream, "image/png", "output.png");
        }
    }
}
