...
public class ShopController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<ActionResult<List<ShopResponseDTO>>> FetchAllShops(
        ...
        [FromQuery] ShopVerificationStatus? verified = null,
    ...
    [HttpPost("Verify")]
    [Authorize]
    public async Task<ActionResult<BaseResponse<ShopResponseDTO>>> VerifyShop([FromBody] ShopVerifyRequestDto request)
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        var result = await mediator.Send(new VerifyShopRequest
        {
            UserId = userId!,
            ShopVerifyRequest = request
        });
        return Ok(result);
    }
    ...
    public async Task<ActionResult<List<ShopResponseDTO>>> GetUnverifiedShops([FromQuery] int skip = 0,
        [FromQuery] int limit = 10)
    {
        var result = await mediator.Send(new GetShopVerificationPendingRequest
        {
            Skip = skip,
            Limit = limit
        });
