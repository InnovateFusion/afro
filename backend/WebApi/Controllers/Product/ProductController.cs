...
namespace backend.WebApi.Controllers.Product
{
    ...
    public class ProductController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ProductResponseDTO>>> FetchAllProducts(
            ...
            [FromQuery] bool? isDeliverable = null,
            [FromQuery] int? availableQuantity = null,
        ...
        [HttpGet("{productId}/ApprovalReview/{approvalId}")]
        [Authorize(Roles = "admin")]
        public async Task<ActionResult<string>> ProductApprovalReview(string productId, int approvalId)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
            var result = await mediator.Send(
                new ProductApproval{ ProductApprovalStatusId = approvalId, ProductId = productId, UserId = userId}
            );
        ...
        [HttpGet("Reviews")]
        [Authorize(Roles = "admin")]
        public async Task<ActionResult<string>> ProductReviews([FromQuery] int skip = 0,
            [FromQuery] int limit = 20)
        {
            var result = await mediator.Send(
                new GetProductListToBeApproved{ Limit = limit, Skip = skip }
            );
