...
class ThemeBloc extends Bloc<ThemeEvent, ThemeState> {
  ...
  ThemeBloc({
    ...
    on<ChangeThemeEvent>((event, emit) {
      ...
      emit(state.copyWith(
          themeMode: event.themeMode, temporary: event.themeMode));
    });

    on<TemporaryThemeEvent>((event, emit) {
      if (event.isDarkMode) {
        emit(state.copyWith(
            temporary: state.themeMode, themeMode: ThemeMode.dark));
      } else {
        emit(state.copyWith(themeMode: state.temporary));
      }
    });
